...
| R04 | Autonomous multi-step decisions | [AgentGraph](../src/browser_agent/graph.py) cycles observation, native tool decision, independent review, optional approval, execution, verification and recovery. Actor receives the ordinary task and observed evidence. | Graph integration is tested with a scripted model substitute. **Current E01/E02 fixture passes:** mail 10 and food 4 passed all checks on fingerprint `924a3ab5d5bc`, including independent semantic review and verified traces. E03/E04 and paid recovery cases remain pending; no operational coaching or real-site success is inferred. Mechanism implementation alone does not prove autonomy quality. |
